...
// --- Trim model --------------------------------------------------------------
// Each animation carries the bar's visual [start, end] on the timeline plus
// `trimStart` / `trimEnd`: how many seconds have been cut from the beginning
// and the end of the underlying clip. Invariant (maintained after first edit):
//   (end - start) + trimStart + trimEnd === clipDuration
// Until Pass 2 hooks up playback, these fields are UI-only: the 3D viewer
// keeps playing the full clip.
function ensureTrim(animation) {
	if (animation.trimStart == null) animation.trimStart = 0
	if (animation.trimEnd == null) animation.trimEnd = 0
}

function getClipDuration(animation) {
	const v = skeletonViewersSig().find((x) => x.plotLabel === animation.name)
	return v?.animationClip?.duration ?? Infinity
}
...
function TimelineAnimationBlock(editor, animation, labelContainer) {
	...
	// --- DRAG block ---
	// Middle drag moves the bar along the timeline preserving its current
	// visual length and its trim values. Only `start`/`end` change.
	dom.addEventListener("mousedown", () => {
		ensureTrim(animation)

		let movementY = 0
		const durFrames = Math.max(
			MIN_FRAMES,
			Math.round((animation.end - animation.start) * FPS)
		)
		...
		function onMouseMove(e) {
			...
			// trimStart / trimEnd unchanged on move
	...
	resizeLeft.addEventListener("mousedown", (event) => {
		...
		function onMouseMove(e) {
			...
			const startOld = animation.start
			let newStart = startOld + dxSec
			const newEnd = endAtDown

			// Snap start to nearest frame
			newStart = snapTime(newStart)

			// Clamp:
			//  - newStart >= 0 (absolute)
			//  - newStart >= startOld - trimStart (can only extend left as far
			//    as previously-trimmed frames allow)
			//  - newStart <= newEnd - MIN_DURATION (keep at least 1 frame)
			const minStart = Math.max(0, startOld - animation.trimStart)
			const maxStart = newEnd - MIN_DURATION
			newStart = Math.max(minStart, Math.min(maxStart, newStart))

			// Propagate the delta into trimStart. Positive delta (dragging
			// right) trims more of the clip's head; negative delta (dragging
			// left) restores previously-trimmed frames.
			const trimDelta = newStart - startOld
			animation.trimStart = Math.max(0, animation.trimStart + trimDelta)
			animation.start = newStart
			animation.end = newEnd
			signals.animationModified.dispatch(animation)
		}
	...
	resizeRight.addEventListener("mousedown", (event) => {
		...
		function onMouseMove(e) {
			...
			const endOld = animation.end
			let newEnd = endOld + dxSec
			const newStart = startAtDown

			// Snap end to nearest frame
			newEnd = snapTime(newEnd)

			// Clamp:
			//  - newEnd >= newStart + MIN_DURATION (keep at least 1 frame)
			//  - newEnd <= endOld + trimEnd (can only extend right as far as
			//    previously-trimmed frames allow; trimEnd must stay >= 0)
			//  - newEnd <= newStart + (clipDuration - trimStart) (hard upper
			//    limit: can't grow past the clip's natural duration)
			const clipDur = getClipDuration(animation)
			const maxEndByTrim = endOld + animation.trimEnd
			const maxEndByClip = newStart + (clipDur - animation.trimStart)
			const maxEnd = Math.min(maxEndByTrim, maxEndByClip)
			const minEnd = newStart + MIN_DURATION
			newEnd = Math.max(minEnd, Math.min(maxEnd, newEnd))

			// Propagate the delta into trimEnd. Positive delta (dragging
			// right) restores previously-trimmed tail frames; negative delta
			// (dragging left) trims more of the clip's tail.
			const trimDelta = endOld - newEnd
			animation.trimEnd = Math.max(0, animation.trimEnd + trimDelta)
			animation.start = newStart
			animation.end = newEnd
			signals.animationModified.dispatch(animation)
		}
...
function TimelineAnimations(editor, labelContainer) {
	...
	// Effective origin = max(bar.start) across all tracks. This is the
	// "action start" point: Pass 2 will clamp playback to begin here. The
	// Timeline widget calls this on animation signals to redraw the gold
	// origin marker and the pre-action overlay.
	container.getOrigin = () => {
		let maxStart = 0
		for (const key in blocks) {
			const a = blocks[key].getAnimation()
			if (a && a.start > maxStart) maxStart = a.start
		}
		return maxStart
	}
