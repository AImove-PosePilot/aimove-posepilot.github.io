...
function Timeline(editor) {
	...
	// Pre-action shading: subtle gold tint covering [0, origin] inside the
	// tracks panel. Lives inside elements.dom so that it scrolls and scales
	// together with the bars. Width is refreshed via updateOrigin().
	const preActionMark = document.createElement("div")
	preActionMark.className = "timeline-preaction"
	elements.dom.appendChild(preActionMark)

	// Gold vertical line marking the effective origin (max bar.start).
	const originMark = document.createElement("div")
	originMark.className = "timeline-origin-mark"
	elements.dom.appendChild(originMark)
	...
	canvas.addEventListener(
		...
		(event) => {
			...
			function onMouseMove(e) {
				const t = clientXToSnappedTime(e)
				editor.setTime(t)

				// Pass 3: drive viewers through the bar-aware time mapper
				// so dragging the red cursor on the ruler moves each viewer
				// to its own clip-local time (taking the bar offset + trim
				// into account) instead of the raw global time.
				runApplyGlobalTime(t)
				skeletonViewersSig().forEach((viewer) => {
					if (viewer.action) viewer.action.paused = false
				})
				setCurrentAnimationTime(t)
	...
	// Scroll updates ruler ticks and time marker position, and mirrors the
	// vertical offset onto the fixed label column so labels and their rows
	// stay visually aligned as a single row while scrolling.
	scroller.addEventListener(
		"scroll",
		() => {
			updateMarks()
			updateTimeMark()
			if (labelContainer.scrollTop !== scroller.scrollTop) {
				labelContainer.scrollTop = scroller.scrollTop
			}
		},
		false
	)

	// Wheel events that happen over the (non-scrollable-by-user) label
	// column should still scroll the whole timeline vertically. Forward
	// them to the main scroller so the user perceives the editor as one
	// row that moves together.
	labelContainer.addEventListener(
		"wheel",
		(event) => {
			// Let ctrl+wheel fall through to the timeline's zoom handler
			// so zoom keeps working even when the cursor is over a label.
	...
	function updateMarks() {
		...
		// Origin-relative labels: the effective origin (max bar.start) is
		// displayed as frame 0; frames before it become negative.
		const originSec =
			typeof elements.getOrigin === "function" ? elements.getOrigin() : 0
		const originF = Math.round(originSec * FPS)

		for (let f = 0; f <= totalFrames; f += labelStepF) {
			const x = (f / FPS) * scale - scroller.scrollLeft + ticksOffset
			const displayF = f - originF
			const label = displayF === 0 ? "0" : String(displayF)
	...
	// Pass 1: UI-only. Whenever bars change we recompute the effective origin
	// and redraw the gold marker + pre-action overlay. This does NOT yet feed
	// playback — that's hooked up in Pass 2 via the Recompile button.
	function updateOrigin() {
		const origin =
			typeof elements.getOrigin === "function" ? elements.getOrigin() : 0
		const widthPx = origin * scale
		preActionMark.style.width = widthPx + "px"
		originMark.style.left = widthPx + "px"
		const visible = origin > 0 ? "" : "none"
		preActionMark.style.display = visible
		originMark.style.display = visible
	}
	...
	signals.animationAdded.add(onBarsChanged)
	signals.animationModified.add(onBarsChanged)
	signals.animationRemoved.add(onBarsChanged)
